# from re_utils import get_certain_attention_matrix, matrix_compress,create_mapping, AttnMapExtractor,hidesep,BFS, build_graph, is_word
from re_utils import BFS,get_certain_attention_matrix, matrix_compress,create_mapping, build_graph,AttnMapExtractor,hidesep
from re_utils import filter_relation_only, filter_tail_only
...
def process_sentence(sentence, tokenizer, nlp,extractor, use_cuda=True):
    ...
    for pair in tail_head_pairs:
        outputs = BFS(pair[1], pair[0], attn_graph, id2token, max(tokenid2word_mapping), black_list_relation, )
